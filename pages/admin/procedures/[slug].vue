    ...
    <div v-else class="flex flex-col items-center justify-center px-[3rem] pb-[0.2rem]">
        <div class="bg-white shadow-lg rounded-2xl p-10 sm:p-[5rem] w-full min-h-screen flex flex-col gap-4">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-center">
                ...
                <a class="btn btn-success" href="/admin/procedures">بازگشت به لیست سندها</a>
            </div>
...
import { useGetProcedureBySlugService } from '~/composables/procetures/useProcedure.service';
import { useGetSubProcessBySlugService } from '~/composables/sub-processes/useSubProcess.service';
const route = useRoute()
const showImage = ref<boolean>(false)
const getProcedure = useGetProcedureBySlugService()
const { data, pending, error } = useAsyncData('procedure' + route.params.slug, () => getProcedure(route.params.slug as string), { server: false })
...
//     try {
//     // دریافت فایل از بک‌اند به صورت blob
...
//     // ایجاد لینک موقت برای دانلود فایل
//     const link = document.createElement('a');
//     link.href = url;
